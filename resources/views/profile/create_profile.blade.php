...
<div class="max-w-6xl mx-auto border border-gray-900 p-8 rounded-2xl shadow-md text-white">
    <h2 class="text-2xl font-bold mb-6">Создать профиль</h2>
    ...
    <form action="{{ route('profile.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ $user->name ?? '' }}" class="w-full border border-gray-700 text-white bg-black rounded-lg p-2.5 @error('name') border-red-500 @enderror">
                @error('name')
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ $user->email ?? '' }}" class="w-full border border-gray-700 text-white bg-black rounded-lg p-2.5 @error('email') border-red-500 @enderror">
                @error('email')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="phone"   id="phone" class="w-full border border-gray-700 text-white bg-black rounded-lg p-2.5 @error('phone') border-red-500 @enderror">
                @error('phone')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="address" id="address" class="w-full border border-gray-700 text-white bg-black rounded-lg p-2.5 @error('address') border-red-500 @enderror">
                @error('address')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
        ...
        <section class="flex items-center justify-around gap-2">
            ...
            <div class="flex flex-col items-center relative">
                ...
                <div class="relative w-48 h-48 bg-gray-800 border-2 border-dashed border-gray-600 rounded-lg flex items-center justify-center">
                    ...
                    <button 
                        type="button" 
                        class="absolute right-[-50px] top-[50%] transform -translate-y-1/2 bg-red-500 hover:bg-red-600 text-white p-2 rounded-lg hidden"
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    window.previewImage = function (event) {
        ...
        if (file) {
            ...
            reader.onload = function () {
                // Set the image source to the uploaded file and display the preview
    ...
    window.removeImage = function () {
        ...
        // Hide the preview and the remove button
        preview.src = '';
